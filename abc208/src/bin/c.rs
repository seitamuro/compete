...
fn main() {
    ...
    let mut keymap = Vec::<(usize, usize)>::new();
    for i in a.into_iter().enumerate() {
        keymap.push(i);
    }
    keymap.sort_by(|a, b| a.1.cmp(&b.1));
    //println!("{:?}", keymap);

    let mut ans = vec![k / n as u64;n];

    for (i, _) in keymap.iter().take((k % n as u64) as usize) {
        ans[*i] += 1;
    }
